...
build(AppInfo) ->
  ...
  AppName = rebar_elixir_utils:to_string(rebar_app_info:name(AppInfo)),

  rebar_elixir_utils:compile(AppDir),

  {ok, Apps} = rebar_utils:list_dir(BuildElixirDir),
  Deps = Apps -- [AppName],
  rebar_elixir_utils:move_to_path(Deps, BuildElixirDir, BuildDir),
  ...
  Lock = rebar_elixir_utils:create_rebar_lock_from_mix(AppDir, Deps),
  ElixirLock = rebar_elixir_utils:elixir_to_lock(Lock),
  rebar_elixir_utils:save_rebar_lock(AppDir, ElixirLock),
  rebar_elixir_utils:delete(filename:join(AppDir, "_build")),
