...
lock(AppInfo, _) ->
  ...
  {hex, rebar_elixir_utils:to_binary(Name), rebar_elixir_utils:to_binary(Vsn)}.
...
fetch({hex, Name_, Vsn_}, CDN, Dir) ->
  Name = rebar_elixir_utils:to_binary(Name_),
  Vsn  = rebar_elixir_utils:to_binary(Vsn_),
